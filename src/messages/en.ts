/**
 * English UI messages.
 * For English-only site: keep this file and set index.ts to export from './en'.
 */
export const messages = {
  site: {
    name: 'MidiDraft',
    title: 'Audio to MIDI Converter — Free MP3 & WAV to MIDI | MidiDraft',
    description:
      'Convert MP3, WAV and other audio to editable MIDI in your browser. Hear the original and the MIDI side by side, clean up the result, and download a standard .mid file free — no account, no upload.',
  },

  nav: {
    features: 'Features',
    converter: 'Converter',
    examples: 'Examples',
    guidesLabel: 'Guides',
    allGuides: {
      title: 'All guides',
      description: 'Every DAW walkthrough, plus background reading',
    },
    pricing: 'Pricing',
    faq: 'FAQ',
    blog: 'Blog',
    product: 'Product',
    resources: 'Resources',
    company: 'Company',
    legal: 'Legal',
    pages: 'Pages',
    batch: {
      title: 'Batch converter',
      description: 'Convert a folder of loops in one run',
    },
    cleanup: {
      title: 'MIDI cleanup',
      description: 'Trim, merge and quantize a transcription',
    },
    guides: {
      flStudio: {
        title: 'Audio to MIDI in FL Studio',
        description: 'Import a converted .mid and set the tempo',
      },
      ableton: {
        title: 'Audio to MIDI in Ableton',
        description: 'Drop a .mid into a MIDI track that plays back right',
      },
      logicPro: {
        title: 'Audio to MIDI in Logic Pro',
        description: 'Flex Pitch, or a converted .mid on an instrument track',
      },
      garageBand: {
        title: 'Audio to MIDI in GarageBand',
        description: 'Get a .mid onto a software instrument track',
      },
      improveResults: {
        title: 'Get better results',
        description: 'Which takes convert well, and which never will',
      },
    },
    about: {
      title: 'About',
      description: 'Learn more about us',
    },
    contact: {
      title: 'Contact',
      description: 'Get in touch',
    },
    waitlist: {
      title: 'Waitlist',
      description: 'Join the waitlist',
    },
    roadmap: {
      title: 'Roadmap',
      description: 'Future plans',
    },
    cookiePolicy: {
      title: 'Cookie Policy',
      description: 'How we use cookies on our site',
    },
    privacyPolicy: {
      title: 'Privacy Policy',
      description: 'How we protect and handle your data',
    },
    termsOfService: {
      title: 'Terms of Service',
      description: 'Legal agreement between you and us',
    },
    changelog: {
      title: 'Changelog',
      description: 'See what we shipped recently',
    },
    ai: {
      title: 'AI',
      summarization: {
        title: 'Text Summarization',
        description: 'Condense long articles with BART',
      },
      translation: {
        title: 'Translation',
        description: 'Translate text across 10 languages',
      },
      tagline: {
        title: 'Tagline Generator',
        description: 'Brainstorm taglines with Llama 3.1',
      },
      tts: {
        title: 'Text to Speech',
        description: 'Synthesize speech with Deepgram Aura',
      },
      caption: {
        title: 'Image Captioning',
        description: 'Describe an image with LLaVA',
      },
      imageFal: {
        title: 'Image Generator · fal.ai',
        description: 'Flux, Gemini, GPT Image 2 via fal.ai',
      },
      imageCf: {
        title: 'Image Generator · Workers AI',
        description: 'Run Flux & SDXL on Cloudflare',
      },
      imageEdit: {
        title: 'Avatar Stylizer',
        description: 'Turn portraits into caricatures',
      },
    },
  },

  changelog: {
    title: 'Changelog',
    subtitle: 'Release notes and product updates',
    description: 'Track every improvement, fix, and new feature we ship',
  },

  roadmap: {
    title: 'Roadmap',
    description: 'See what we are building next and what is on the horizon',
    subtitle: 'Explore our future plans and upcoming features',
    columns: {
      backlog: 'Backlog',
      inProgress: 'In Progress',
      done: 'Done',
    },
  },

  footer: {
    tagline:
      'Turn audio ideas into editable MIDI drafts, then clean and export them for your DAW.',
    rightsReserved: 'All Rights Reserved',
  },

  auth: {
    common: {
      signup: 'Sign up',
      login: 'Log in',
      logout: 'Log out',
      logoutFailed: 'Log out failed',
      byClickingContinue: 'By clicking continue, you agree to our ',
      termsOfService: 'Terms of Service',
      privacyPolicy: 'Privacy Policy',
      and: ' and ',
    },
    error: {
      title: 'Oops! Something went wrong!',
      tryAgain: 'Please try again',
      backToLogin: 'Back to login',
      /** Friendly labels for Better Auth error codes (key = code, value = message) */
      codes: {
        signup_disabled: 'Sign up is disabled.',
        account_already_linked_to_different_user:
          'This account is already linked to another user.',
        unable_to_link_account: 'Unable to link account.',
        unable_to_get_user_info: 'Could not get user info from provider.',
        "email_doesn't_match": "Email doesn't match.",
        email_not_found: 'Email not found.',
        oauth_provider_not_found: 'Sign-in provider not found.',
        no_callback_url: 'Missing callback URL.',
        no_code: 'Authorization code missing.',
        state_mismatch: 'Session expired or invalid. Please try again.',
        state_not_found: 'Session not found. Please try again.',
        invalid_callback_request: 'Invalid sign-in request. Please try again.',
        verification_token_expired:
          'This verification link has expired. Please request a new one.',
        verification_failed:
          'Invalid verification token. Please request a new verification email.',
        email_not_verified:
          'Email is not verified. Please sign in again to receive a new verification email.',
      } as Record<string, string>,
    },
    social: {
      or: 'Or continue with',
      signInWithGoogle: 'Sign In with Google',
    },
    login: {
      title: 'Sign In',
      description: 'Sign in to your account to continue',
      welcomeBack: 'Welcome back',
      email: 'Email',
      password: 'Password',
      signIn: 'Sign In',
      signUpHint: "Don't have an account? Sign up",
      forgotPassword: 'Forgot Password?',
      showPassword: 'Show password',
      hidePassword: 'Hide password',
      emailRequired: 'Please enter your email',
      passwordRequired: 'Please enter your password',
      placeholderEmail: 'name@example.com',
      placeholderPassword: '******',
    },
    forgotPassword: {
      title: 'Forgot Password',
      description: 'Request a password reset link to be sent to your email',
      email: 'Email',
      send: 'Send reset link',
      backToLogin: 'Back to login',
      checkEmail: "We've sent a password reset link to your email",
      emailRequired: 'Please enter your email',
      placeholderEmail: 'name@example.com',
    },
    register: {
      title: 'Sign Up',
      description: 'Create an account to get started',
      createAccount: 'Create an account',
      name: 'Name',
      email: 'Email',
      password: 'Password',
      signUp: 'Sign Up',
      signInHint: 'Already have an account? Sign in',
      checkEmail: 'Check your email to verify your account',
      showPassword: 'Show password',
      hidePassword: 'Hide password',
      nameRequired: 'Please enter your name',
      emailRequired: 'Please enter your email',
      passwordRequired: 'Please enter your password',
      placeholderName: 'name',
      placeholderEmail: 'name@example.com',
      placeholderPassword: '******',
    },
    resetPassword: {
      title: 'Reset Password',
      description: 'Set a new password for your account',
      password: 'Password',
      reset: 'Reset password',
      backToLogin: 'Back to login',
      showPassword: 'Show password',
      hidePassword: 'Hide password',
      minLength: 'Password must be at least 8 characters',
      invalidToken: 'Invalid or expired reset link, please request a new one',
      placeholderPassword: '******',
    },
  },

  dashboard: {
    title: 'Dashboard',
    sidebar: {
      dashboard: 'Dashboard',
      settings: 'Settings',
      profile: 'Profile',
      billing: 'Billing',
      security: 'Security',
      apiKeys: 'API Keys',
      files: 'Files',
      notifications: 'Notifications',
    },
    avatar: {
      dashboard: 'Dashboard',
      billing: 'Billing',
      settings: 'Settings',
    },
    sectionCards: {
      totalRevenue: 'Total Revenue',
      newCustomers: 'New Customers',
      activeAccounts: 'Active Accounts',
      growthRate: 'Growth Rate',
      trendingUp: 'Trending up this month',
      visitors: 'Visitors for the last 6 months',
      downPeriod: 'Down 20% this period',
      acquisition: 'Acquisition needs attention',
      retention: 'Strong user retention',
      engagement: 'Engagement exceed targets',
      growth: 'Growth rate improved',
      growthFooter: 'Steady performance increase',
      growthSub: 'Meets growth projections',
    },
  },

  settings: {
    profile: {
      title: 'Profile',
      description: 'Manage your account information',
      name: {
        title: 'Name',
        description: 'Please enter your display name',
        placeholder: 'Enter your name',
        hint: 'Please use 3-30 characters for your name',
        saving: 'Saving...',
        save: 'Save',
        minLength: 'Please use 3 characters at minimum',
        maxLength: 'Please use 30 characters at maximum',
        success: 'Name updated successfully',
        fail: 'Could not update your name. Please try again.',
      },
      avatar: {
        title: 'Avatar',
        description: 'Click upload button to upload a custom one',
        uploadNotConfigured: 'Upload Avatar (storage not configured)',
        uploadAvatar: 'Upload Avatar',
        uploading: 'Uploading...',
        success: 'Avatar updated successfully',
        fail: 'Could not update your avatar. Please try again.',
        hint: 'An avatar is optional but strongly recommended',
      },
    },
    security: {
      title: 'Security',
      description: 'Manage your security settings',
      updatePassword: {
        title: 'Change Password',
        description: 'Enter your current password and a new password',
        currentPassword: 'Current Password',
        newPassword: 'New Password',
        currentRequired: 'Current password is required',
        newMinLength: 'Password must be at least 8 characters',
        hint: 'Please use at least 8 characters for password',
        showPassword: 'Show password',
        hidePassword: 'Hide password',
        placeholderCurrent: 'Current Password',
        placeholderNew: 'New Password',
        saving: 'Saving...',
        save: 'Save',
        success: 'Password updated successfully',
        fail: 'Could not update your password. Please check your current password and try again.',
      },
      deleteAccount: {
        title: 'Delete Account',
        description: 'Permanently remove your account and all of its contents',
        warning:
          'This action is not reversible, so please continue with caution',
        button: 'Delete Account',
        confirmTitle: 'Delete Account',
        confirmDescription:
          'Are you sure you want to delete your account? This action cannot be undone.',
        cancel: 'Cancel',
        deleting: 'Deleting...',
        confirm: 'Delete',
        success: 'Account deleted successfully',
        fail: 'Could not delete your account. Please try again or contact support.',
      },
      resetPassword: {
        title: 'Reset Password',
        description: 'Reset password to enable email login',
        info: 'Resetting your password will allow you to sign in using your email and password in addition to your social login methods. You will receive an email with instructions to reset your password.',
        button: 'Reset Password',
      },
    },
    apiKeys: {
      title: 'API Keys',
      description: 'Manage your API keys',
      createButton: 'Create API Key',
      createDialogTitle: 'Create API Key',
      createDialogDescription: 'Give your API key a name to identify it later',
      keyNameLabel: 'Name',
      keyNamePlaceholder: 'Enter a name for your API key',
      cancel: 'Cancel',
      create: 'Create',
      creating: 'Creating…',
      newKeyDialogTitle: 'Your new API key',
      newKeyDialogDescription:
        'Copy this key now, you will not be able to see it again',
      done: 'Done',
      delete: 'Delete',
      never: 'Never',
      noResults: 'No API keys found',
      createSuccess: 'API key created successfully',
      createError: 'Could not create the API key. Please try again.',
      deleteSuccess: 'API key deleted successfully',
      deleteError: 'Could not delete the API key. Please try again.',
      columns: {
        name: 'Name',
        key: 'Key',
        createdAt: 'Created at',
        expiresAt: 'Expires at',
        actions: 'Actions',
      },
    },
    files: {
      title: 'Files',
      description: 'Manage your uploaded files',
      uploadButton: 'Upload file',
      uploadDialogTitle: 'Upload file',
      uploadDialogDescription: 'File will be stored under your account',
      fileLabel: 'File',
      isPublicLabel: 'Public (anyone with link can view)',
      descriptionLabel: 'Description',
      descriptionPlaceholder: 'file description (optional)',
      cancel: 'Cancel',
      upload: 'Upload',
      uploading: 'Uploading…',
      delete: 'Delete',
      noResults: 'No files yet',
      openLink: 'Open',
      uploadSuccess: 'File uploaded successfully',
      uploadError:
        'Could not upload the file. Please check the file and try again.',
      deleteSuccess: 'File deleted successfully',
      deleteError: 'Could not delete the file. Please try again.',
      columns: {
        originalName: 'Name',
        contentType: 'Type',
        size: 'Size',
        createdAt: 'Uploaded',
        isPublic: 'Access',
        accessLink: 'Link',
        actions: 'Actions',
      },
    },
    notification: {
      title: 'Notification',
      description: 'Manage your notification preferences',
      newsletter: {
        title: 'Newsletter Subscription',
        description: 'Manage your newsletter subscription preferences',
        label: 'Subscribe to newsletter',
        hint: 'You can change your subscription preferences at any time',
        emailRequired: 'Email is required to subscribe to the newsletter',
        subscribeSuccess: 'Successfully subscribed to the newsletter',
        unsubscribeSuccess: 'Successfully unsubscribed from the newsletter',
        error: 'An error occurred while updating your subscription',
      },
    },
    billing: {
      title: 'Billing',
      description: 'Manage your subscription and billing',
      breadcrumb: 'Billing',
      card: {
        currentPlan: 'Current plan',
        currentPlanDescription: 'Your current plan and subscription status',
        free: 'Free',
        lifetime: 'Lifetime',
        loadError: 'Failed to load billing info',
        retry: 'Retry',
        noPlan: 'No plan selected',
        upgradePlan: 'Upgrade plan',
        freePlanMessage: 'Upgrade to a paid plan to unlock more features',
        lifetimeMessage: 'You have lifetime access to all features',
        passActiveUntil: 'Active until',
        passMessage: 'One payment, does not renew.',
        periodStart: 'Current period starts:',
        periodEnds: 'Current period ends:',
        cancelsAtPeriodEnd: '(cancels at period end)',
        trialEnds: 'Trial ends:',
        upgradeCta: 'Upgrade to a paid plan to unlock more features',
        manageBilling: 'Manage billing',
        manageSubscription: 'Manage subscription',
        viewPlans: 'View plans',
        statusTrial: 'Trial',
        statusActive: 'Active',
      },
    },
    payment: {
      processing: {
        title: 'Confirming your payment',
        description: 'Please wait while we verify your payment',
      },
      success: {
        title: 'Payment confirmed',
        description: 'Redirecting you to your account...',
      },
      failed: {
        title: 'Invalid session',
        description: 'Please try again from the pricing page',
      },
      timeout: {
        title: 'Taking longer than usual',
        description:
          'You can close this page and check your billing page for the result',
      },
    },
  },

  admin: {
    title: 'Admin',
    users: {
      title: 'Users',
      search: 'Search by name or email…',
      clearSearch: 'Clear search',
      admin: 'Admin',
      user: 'User',
      active: 'Active',
      inactive: 'Inactive',
      banned: 'Banned',
      emailCopied: 'Email copied to clipboard',
      joined: 'Joined',
      updated: 'Updated',
      close: 'Close',
      columns: {
        name: 'Name',
        email: 'Email',
        role: 'Role',
        createdAt: 'Created at',
        status: 'Status',
        banReason: 'Ban reason',
        banExpires: 'Ban expires',
      },
      noResults: 'No users found',
      ban: {
        reason: 'Ban reason',
        reasonPlaceholder: 'Enter reason for banning this user…',
        expires: 'Ban expires',
        selectDate: 'Select date (optional)',
        never: 'Never',
        defaultReason: 'Spamming',
        button: 'Ban user',
        success: 'User has been banned.',
        error: 'Failed to ban user.',
      },
      unban: {
        button: 'Unban user',
        success: 'User has been unbanned.',
        error: 'Failed to unban user.',
      },
    },
  },

  pricing: {
    title: 'Pricing',
    description:
      'The free converter is the whole job: convert, listen, adjust, download. Paid plans buy time back on repetitive batches — nothing else.',
    subtitle: 'Free stays free. Pay only when you have a pile of clips.',
    monthly: 'Monthly',
    yearly: 'Yearly',
    card: {
      popular: 'Popular',
      currentPlan: 'Current Plan',
      free: 'Free',
      perMonth: '/month',
      perYear: '/year',
      notAvailable: 'Not Available',
      getStartedForFree: 'Get Started For Free',
      yourCurrentPlan: 'Your Current Plan',
      getLifetimeAccess: 'Get Lifetime Access',
      getStarted: 'Get Started',
      daysFreeTrial: 'days free trial',
    },
    checkout: {
      failed: 'Failed to open checkout page',
      loading: 'Loading...',
    },
    customerPortal: {
      failed: 'Failed to open customer portal',
      loading: 'Loading...',
    },
    plans: {
      free: {
        name: 'Free',
        description: 'The whole conversion, start to finish. No account.',
        features: [
          'One file at a time, up to 3 minutes',
          'Original / MIDI A-B playback',
          'Piano roll with note count and range',
          'Sensitivity, note length and pitch range controls',
          'Standard .mid download with correct tempo',
          'Runs in your browser — audio is never uploaded',
        ],
        limits: ['Batch processing', 'Saved cleanup presets', 'ZIP export'],
      },
      pass: {
        name: 'Project Pass',
        description:
          'For clearing one project off your desk. One-time, does not renew.',
        features: [
          'Everything in Free',
          'Batch up to 10 clips in one run',
          'One set of cleanup settings across the batch',
          'Consistent file naming and ZIP download',
          'Failed clips retry without redoing the batch',
          '7 days, one payment, no auto-renewal',
        ],
        limits: ['Saved presets across projects'],
      },
      pro: {
        name: 'Pro',
        description: 'For producers converting material every week.',
        features: [
          'Everything in Project Pass, ongoing',
          'Saved cleanup presets',
          'Presets sync to your account',
          'Cancel any time from your account page',
        ],
        limits: [],
      },
    },
  },

  blog: {
    title: 'Blog',
    description:
      'Product updates, and background on how audio to MIDI conversion works',
    noPosts: 'No posts yet',
    allPosts: 'All posts',
    previous: 'Previous',
    next: 'Next',
    page: 'Page',
    of: 'of',
  },

  about: {
    title: 'About',
    description:
      'Who builds MidiDraft, what it does today, and what it deliberately does not do',
    bio: 'A focused audio-to-MIDI tool for producers',
    introduction:
      'MidiDraft is a one-person project. It does one thing: turn a short audio idea — a piano line, a guitar loop, a hummed melody — into a MIDI draft you can open in your DAW and keep working on. The conversion runs in your browser, so your unreleased material never leaves your machine. It is not a transcription service, not a stem separator, and not a sheet-music engine. If it cannot give you something worth editing, it should say so rather than sell you a subscription.',
    followUs: 'Follow us on X',
    contactUs: 'Contact us',
  },

  contact: {
    title: 'Contact',
    description:
      'Bug reports, material that will not convert, and questions about what this tool can do — all welcome.',
    formTitle: 'Send message',
    name: 'Name',
    email: 'Email',
    message: 'Message',
    placeholderName: 'Your name',
    placeholderEmail: 'you@example.com',
    placeholderMessage: 'Your message',
    nameMin: 'Name must be at least 3 characters',
    nameMax: 'Name must not exceed 30 characters',
    emailInvalid: 'Please enter a valid email address',
    messageMin: 'Message must be at least 10 characters',
    messageMax: 'Message must not exceed 500 characters',
    send: 'Send',
    sending: 'Sending…',
    error: 'Failed to send the message',
  },

  waitlist: {
    title: 'Waitlist',
    description: 'Join the waitlist for early access and updates',
    formTitle: 'Join the waitlist',
    email: 'Email',
    placeholderEmail: 'you@example.com',
    subscribe: 'Subscribe',
    subscribing: 'Subscribing…',
    emailInvalid: 'Please enter a valid email address',
    error: 'Failed to subscribe',
  },

  newsletter: {
    title: 'Newsletter',
    subtitle: 'Join the community',
    description: 'Subscribe to our newsletter for the latest news and updates',
    email: 'Email',
    placeholderEmail: 'you@example.com',
    emailInvalid: 'Please enter a valid email address',
    subscribe: 'Subscribe',
    error: 'Failed to subscribe to the newsletter',
    thanks: 'Thanks for subscribing!',
  },

  mail: {
    layout: {
      team: 'Team',
      copyright: 'All Rights Reserved',
    },
    contactMessage: {
      subject: 'Contact Message from Website',
      name: 'Name:',
      email: 'Email:',
      message: 'Message:',
    },
    forgotPassword: {
      subject: 'Reset your password',
      greeting: 'Hi,',
      body: 'Please click the link below to reset your password.',
      button: 'Reset password',
    },
    subscribeNewsletter: {
      subject: 'Thanks for subscribing',
      title: 'Thanks for subscribing',
      body: 'Thank you for subscribing to the newsletter. We will keep you updated with the latest news and updates.',
    },
    verifyEmail: {
      subject: 'Verify your email',
      greeting: 'Hi,',
      body: 'Please click the link below to verify your email address.',
      button: 'Confirm email',
    },
  },

  notFound: {
    title: 'Not Found',
    description: 'The page you are looking for does not exist',
    backToHome: 'Back to Home',
  },

  catchBoundary: {
    title: 'Something went wrong!',
    description: 'We encountered an unexpected error. Please try again.',
    backToHome: 'Back to Home',
  },

  common: {
    cancel: 'Cancel',
    loading: 'Loading...',
    settings: 'Settings',
    mode: {
      theme: 'Theme',
      light: 'Light',
      dark: 'Dark',
      system: 'System',
    },
    table: {
      ascending: 'Ascending',
      descending: 'Descending',
      resetSorting: 'Reset sorting',
      hideColumn: 'Hide column',
      rowsPerPage: 'Rows per page',
      page: 'Page',
      firstPage: 'First page',
      previousPage: 'Previous page',
      nextPage: 'Next page',
      lastPage: 'Last page',
      viewOptions: 'View options',
      searchColumns: 'Search columns…',
      noColumns: 'No columns.',
      clearFilters: 'Clear filters',
      noResultsFound: 'No results found.',
      selected: 'selected',
      clearSelection: 'Clear selection',
      // sort-list
      sort: 'Sort',
      sortBy: 'Sort by',
      noSortingApplied: 'No sorting applied',
      addSortingHint: 'Add sorting to organize your rows.',
      modifySortingHint: 'Modify sorting to organize your rows.',
      addSort: 'Add sort',
      searchFields: 'Search fields…',
      noFieldsFound: 'No fields found.',
      // filter-list
      filter: 'Filter',
      filters: 'Filters',
      noFiltersApplied: 'No filters applied',
      addFiltersHint: 'Add filters to refine your rows.',
      modifyFiltersHint: 'Modify filters to refine your rows.',
      addFilter: 'Add filter',
      resetFilters: 'Reset filters',
      selectField: 'Select field',
      where: 'Where',
      enterValue: 'Enter a value…',
      pickDate: 'Pick a date',
      selectOption: 'Select option…',
      selectOptions: 'Select options…',
      searchOptions: 'Search options…',
      noOptionsFound: 'No options found.',
      boolTrue: 'True',
      boolFalse: 'False',
      selectJoinOperator: 'Select join operator',
      // config labels
      opContains: 'Contains',
      opNotContains: 'Does not contain',
      opIs: 'Is',
      opIsNot: 'Is not',
      opIsEmpty: 'Is empty',
      opIsNotEmpty: 'Is not empty',
      opLessThan: 'Is less than',
      opLessThanOrEqual: 'Is less than or equal to',
      opGreaterThan: 'Is greater than',
      opGreaterThanOrEqual: 'Is greater than or equal to',
      opIsBetween: 'Is between',
      opIsBefore: 'Is before',
      opIsAfter: 'Is after',
      opIsOnOrBefore: 'Is on or before',
      opIsOnOrAfter: 'Is on or after',
      opIsRelativeToToday: 'Is relative to today',
      opHasAnyOf: 'Has any of',
      opHasNoneOf: 'Has none of',
      sortAsc: 'Asc',
      sortDesc: 'Desc',
      // data-table-action-bar
      noResults: 'No results.',
      // data-table-date-filter
      selectDateRange: 'Select date range',
      selectDate: 'Select date',
      clearFilterLabel: 'Clear {title} filter',
      // data-table-range-filter
      minimumValue: 'minimum value',
      maximumValue: 'maximum value',
      to: 'to',
      // data-table-slider-filter
      from: 'From',
      slider: 'slider',
      clear: 'Clear',
      // data-table-filter-menu
      resetAllFilters: 'Reset all filters',
      openFilterMenu: 'Open filter command menu',
      typeToAddFilter: 'Type to add filter…',
      filterByValue: 'Filter by',
      empty: 'empty',
      notEmpty: 'not empty',
      pickDateEllipsis: 'Pick date…',
    },
  },
} as const;
